...
import ingv.dao.INGVEventDAO;
import ingv.dao.INGVEventDAOPostgres;
...
public class Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        eventDAO = new INGVEventDAOPostgres();
    ...
    private LoadReportService loaderInit() {
        ...
        loader.setOnSucceeded(event -> {
            ...
            try{
                ...
                e.printStackTrace();
                Utilities.showDialog(Alert.AlertType.ERROR, "Error", "Error saving events on DB");
            }
            loader.reset();
        });
    ...
    private void saveEventOnDB() throws Exception{
        for(INGVEvent event : events){
            eventDAO.saveEvent(event);
        }
    }

    private void loadEventFromDB() throws Exception{
        events.setAll(eventDAO.getAllEvents());
        System.out.println(PrintColors.format("Data successfully loaded from DB", PrintColors.GREEN));
    }
}
