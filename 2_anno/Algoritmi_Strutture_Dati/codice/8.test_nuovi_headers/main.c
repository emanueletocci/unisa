...
TBinaryTree HT_to_BST(THashTable *ht, TBinaryTree bst) {
    for (int i = 0; i < ht->bucket_number; i++) {
        for (TNode *node = ht->bucket[i]; node != NULL; node = node->link){
            binarytree_insert(bst , node -> info);
        }
    }
    return bst;
}
...
TList BST_first_to_list(TBinaryTree bst, TList list, int *n) {
    TBinaryTree min;
    for (int i = 0 ; i < 6 ; i++){
        min = binarytree_search_min(bst);
        list = list_insert(list, min -> info);
        bst = binarytree_delete(bst, min -> info);
    }
    return list;
}
...
int main() {
    ...
    int n = 6;
    TList elenco3 = list_create();
    printf("\nRiverso in elenco3 i primi %d libri di elenco1 e rimuovo i codici pari.", n);
    printf("\nLibri in elenco3 (Lista):\n");
